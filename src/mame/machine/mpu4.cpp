...
void mpu4_bwboki_state::mpu4_install_mod4bwb_space(address_space &space)
{
	space.install_readwrite_handler(0x0810, 0x0810, read8_delegate(FUNC(mpu4_bwboki_state::bwb_characteriser_r),this),write8_delegate(FUNC(mpu4_bwboki_state::bwb_characteriser_w),this));
...
MACHINE_START_MEMBER(mpu4_mod4oki_state,mpu4oki)
{
	address_space &space = m_maincpu->space(AS_PROGRAM);
	mpu4_config_common();
...
MACHINE_START_MEMBER(mpu4_bwboki_state,mpu4bwb)
{
	address_space &space = m_maincpu->space(AS_PROGRAM);
	mpu4_config_common();
...
MACHINE_START_MEMBER(mpu4_mpu4crys_state,mpu4cry)
{
	mpu4_config_common();
...
mpu4_state::mpu4_chr_table mpu4_mod4oki_state::s_andycp10c_data[72] = {
...
mpu4_state::mpu4_chr_table mpu4_mod2_state::s_ccelbr_data[72] = {
...
mpu4_state::mpu4_chr_table mpu4_mod4yam_state::s_gmball_data[72] = {
...
mpu4_state::mpu4_chr_table mpu4_state::s_grtecp_data[72] = {
...
mpu4_state::mpu4_chr_table mpu4_mod4oki_alt_state::s_oldtmr_data[72] = {
...
const mpu4_state::bwb_chr_table mpu4_bwboki_state::s_blsbys_data1[5] = {
//Magic number 724A
...
mpu4_state::mpu4_chr_table mpu4_bwboki_state::s_blsbys_data[8] = {
...
DRIVER_INIT_MEMBER(mpu4_mod4oki_state,m4_andycp10c)
{
	DRIVER_INIT_CALL(m4default);
	DRIVER_INIT_CALL(m4_small_extender);
	m_current_chr_table = s_andycp10c_data;
}

DRIVER_INIT_MEMBER(mpu4_mod4oki_alt_state,m_oldtmr)
{
	DRIVER_INIT_CALL(m4_six_reel_std);
	DRIVER_INIT_CALL(m4default_banks);

	m_current_chr_table = s_oldtmr_data;
}

DRIVER_INIT_MEMBER(mpu4_mod2_state,m_ccelbr)
{
	DRIVER_INIT_CALL(m4default);
	m_current_chr_table = s_ccelbr_data;
}

DRIVER_INIT_MEMBER(mpu4_mod4yam_state,m4gambal)
{
	DRIVER_INIT_CALL(m4default);
	m_current_chr_table = s_gmball_data;
}

DRIVER_INIT_MEMBER(mpu4_state,m_grtecp)
{
	m_current_chr_table = s_grtecp_data;
}

DRIVER_INIT_MEMBER(mpu4_bwboki_state,m_blsbys)
{
	m_bwb_bank=1;
	DRIVER_INIT_CALL(m4_five_reel_std);
	m_bwb_chr_table1 = s_blsbys_data1;
	m_current_chr_table = s_blsbys_data;
...
READ8_MEMBER(mpu4_mpu4crys_state::crystal_sound_r)
{
	return machine().rand();
}
//this may be a YMZ280B
WRITE8_MEMBER(mpu4_mpu4crys_state::crystal_sound_w)
{
	printf("crystal_sound_w %02x\n",data);
}

DRIVER_INIT_MEMBER(mpu4_mpu4crys_state,m_frkstn)
{
	address_space &space = m_maincpu->space(AS_PROGRAM);
	DRIVER_INIT_CALL(m4default_big);
	space.install_read_handler(0x0880, 0x0880, read8_delegate(FUNC(mpu4_mpu4crys_state::crystal_sound_r),this));
	space.install_write_handler(0x0881, 0x0881, write8_delegate(FUNC(mpu4_mpu4crys_state::crystal_sound_w),this));
}
...
DRIVER_INIT_MEMBER(mpu4_mpu4crys_state,crystal)
{
	DRIVER_INIT_CALL(m_frkstn);
	descramble_crystal(memregion( "maincpu" )->base(), 0x0000, 0x10000, 0x00);
}

DRIVER_INIT_MEMBER(mpu4_mpu4crys_state,crystali)
{
	DRIVER_INIT_CALL(m_frkstn);
...
MACHINE_CONFIG_START(mpu4_bwboki_state::mpu4_bwb_5reel)
	MCFG_MPU4_BWB_REEL_ADD("reel0")
	MCFG_STEPPER_OPTIC_CALLBACK(WRITELINE(mpu4_state, reel0_optic_cb))
	MCFG_MPU4_BWB_REEL_ADD("reel1")
	MCFG_STEPPER_OPTIC_CALLBACK(WRITELINE(mpu4_state, reel1_optic_cb))
...
MACHINE_CONFIG_START(mpu4_mod2_state::device_add_mconfig)
	mpu4base(config);
...
MACHINE_CONFIG_START(mpu4_mod2_alt_state::device_add_mconfig)
	mpu4base(config);
	MCFG_SOUND_ADD("ay8913", AY8913, MPU4_MASTER_CLOCK/4)
	MCFG_AY8910_OUTPUT_TYPE(AY8910_SINGLE_OUTPUT)
	MCFG_AY8910_RES_LOADS(820, 0, 0)
...
MACHINE_CONFIG_START(mpu4_mod4yam_state::device_add_mconfig)
	mpu4base(config);
...
MACHINE_CONFIG_START(mpu4_mod4oki_state::device_add_mconfig)
	mpu4base(config);
	MCFG_MACHINE_START_OVERRIDE(mpu4_mod4oki_state, mpu4oki)
...
MACHINE_CONFIG_START(mpu4_mod4oki_alt_state::device_add_mconfig)
	mpu4base(config);
	MCFG_MACHINE_START_OVERRIDE(mpu4_mod4oki_alt_state, mpu4oki)
...
MACHINE_CONFIG_START(mpu4_mod4oki_5r_state::device_add_mconfig)
	mpu4base(config);
	MCFG_MACHINE_START_OVERRIDE(mpu4_mod4oki_5r_state, mpu4oki)
...
MACHINE_CONFIG_START(mpu4_bwboki_state::device_add_mconfig)
	mpu4base(config);
	MCFG_MACHINE_START_OVERRIDE(mpu4_bwboki_state,mpu4bwb)
	mpu4_common2(config);
...
MACHINE_CONFIG_START(mpu4_mpu4crys_state::device_add_mconfig)
	mpu4_mod2_state::device_add_mconfig(config);
	MCFG_MACHINE_START_OVERRIDE(mpu4_mpu4crys_state,mpu4cry)
